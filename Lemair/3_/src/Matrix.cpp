...
Matrix::Matrix(int lines, int columns, double* data)
    : _columns(columns), _lines(lines), _data(data) {
}

Matrix::Matrix(const Matrix& matrix)
    : _columns(matrix._columns), _lines(matrix._lines) {
    _data = new double[_columns * _lines]();
    memcpy(_data, matrix._data, sizeof(double) * _lines * _columns);
}

Matrix::Matrix(Matrix&& rmatrix)
    : _columns(rmatrix._columns), _lines(rmatrix._lines), _data(rmatrix._data) {
    cout << "rrrMatrix(&&)" << endl;
...
double Matrix::get(const int line, const int column) const
{
    return _data[line * _columns + column];
}
void Matrix::set(const int line, const int column, const double value)
{
    _data[line * _columns + column] = value;
}
